...
*Amazon Bedrock AgentCore Security Controls*::
Enforces VPC isolation and encryption requirements for https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/what-is-bedrock-agentcore.html[Amazon Bedrock AgentCore] resources deployed in infrastructure accounts:

* *VPC-Only Resource Creation*: Denies creation or update of AgentCore Runtimes, Code Interpreters, and Browsers unless https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/security-vpc-condition.html[VPC subnet and security group condition keys] (`bedrock-agentcore:subnets`, `bedrock-agentcore:securityGroups`) are provided. This ensures all AgentCore compute resources run within a VPC rather than on public infrastructure.
* *VPC-Only Invocation*: Denies invocation of AgentCore Runtimes when the request does not originate from a VPC (using the https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html#condition-keys-sourcevpc[`aws:SourceVpc`] global condition key). Requests made by AWS services on behalf of the caller (indicated by https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html#condition-keys-viaawsservice[`aws:ViaAWSService`]) are permitted to allow AgentCore service-to-service communication.
* *Memory Encryption Requirement*: Denies creation of AgentCore Memory resources unless a customer-managed KMS key is specified (using the https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazonbedrockagentcore.html#amazonbedrockagentcore-policy-keys[`bedrock-agentcore:KmsKeyArn`] condition key), ensuring all agent memory data is encrypted with organization-controlled keys.
...
*Amazon Bedrock AgentCore Security Controls*::
Enforces VPC isolation and encryption requirements for https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/what-is-bedrock-agentcore.html[Amazon Bedrock AgentCore] resources deployed in workload accounts:

* *VPC-Only Resource Creation* (Dev/Test/Prod only): Denies creation or update of AgentCore Runtimes, Code Interpreters, and Browsers unless https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/security-vpc-condition.html[VPC subnet and security group condition keys] (`bedrock-agentcore:subnets`, `bedrock-agentcore:securityGroups`) are provided. This ensures all AgentCore compute resources run within a VPC rather than on public infrastructure.
* *VPC-Only Invocation* (Dev/Test/Prod only): Denies invocation of AgentCore Runtimes when the request does not originate from a VPC (using the https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html#condition-keys-sourcevpc[`aws:SourceVpc`] global condition key). Requests made by AWS services on behalf of the caller (indicated by https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_condition-keys.html#condition-keys-viaawsservice[`aws:ViaAWSService`]) are permitted to allow AgentCore service-to-service communication.
* *Memory Encryption Requirement* (all workloads including Sandbox): Denies creation of AgentCore Memory resources unless a customer-managed KMS key is specified (using the https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazonbedrockagentcore.html#amazonbedrockagentcore-policy-keys[`bedrock-agentcore:KmsKeyArn`] condition key), ensuring all agent memory data is encrypted with organization-controlled keys.

+
NOTE: The Sandbox OU intentionally omits VPC enforcement for AgentCore to allow developers to iterate quickly using public endpoints. Only the Memory CMK requirement is enforced in Sandbox, maintaining consistent key-usage patterns across all tiers while preserving development agility.

These guardrails are consistently applied across the entire workload hierarchy—covering Sandbox, Development, Test, and Production organizational units—creating standardized security boundaries regardless of environment maturity. The controls focus specifically on protecting network architecture, enforcing data encryption, and securing AI agent infrastructure while allowing teams sufficient flexibility to deploy and manage application resources within these secure boundaries.
...
==== Container Deployment Account Protection with Deployment Guardrails

The LZA Universal Configuration applies two preventive guardrails to the LzaDeployment OU for accounts hosting container-based LZA deployments.

NOTE: These SCPs are applicable only to container-based LZA deployments in partitions that require it, such as the AWS European Sovereign Cloud. Standard AWS CodePipeline-based deployments do not require these policies.

*Operator Actions Permitted*::
Operators in the LzaDeployment account can:

* Upload configuration files to the Amazon S3 configuration bucket
* Trigger LZA deployments through AWS Systems Manager automation (`RunEngine`)
* Update the InstallerContainerStack through AWS CloudFormation
* Monitor deployment status through Amazon CloudWatch Logs, AWS CloudFormation, and Amazon ECS

*LZA Resource Modification Protection (Guardrails 1)*::
Prevents unauthorized modification of pipeline-deployed LZA resources including AWS Lambda functions, AWS IAM roles, AWS CloudFormation stacks, AWS Systems Manager parameters, Amazon S3 buckets, Amazon CloudWatch log groups, Amazon SNS topics, Amazon Kinesis/Amazon Data Firehose streams, and Amazon EventBridge rules. Prevents disabling or reconfiguration of Amazon GuardDuty, AWS Security Hub, and Amazon Macie. Denies all actions for the root account.

*Networking, Encryption, and Deletion Protection (Guardrails 2)*::
Prevents unauthorized networking changes, enforces storage encryption, and protects InstallerContainerStack resources from deletion:

* Blocks Amazon VPC, subnet, NAT gateway, internet gateway, transit gateway, route, and IPAM modifications
* Requires encryption for Amazon EFS file systems, Amazon RDS instances, and Amazon Aurora clusters
* Blocks deletion of resources tagged with the accelerator prefix or qualifier

The implementation includes exception conditions that allow authorized roles (accelerator deployment roles and management access roles) to make legitimate changes while blocking all unauthorized modification attempts.

=== Data Perimeter Controls (RCPs)

link:https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_policies_rcps.html[Resource Control Policies (RCPs)] complement SCPs by applying guardrails directly to AWS resources rather than to identities. Where SCPs constrain what principals in your organization can do, RCPs constrain what can be done to your resources—regardless of who makes the request. Together they form the two sides of an link:https://aws.amazon.com/identity/data-perimeters-on-aws/[AWS data perimeter]: SCPs protect the identity boundary, RCPs protect the resource boundary.

The LZA Universal Configuration deploys a single RCP (`lza-core-rcp-guardrails-1.json`) attached to the Infrastructure, Security, and Workloads OUs. It implements three data perimeter objectives across link:https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_policies_rcps.html#rcp-supported-services[all RCP-supported services]:

==== Identity Perimeter — Blocking External Principals

Four statements deny direct access by principals outside the AWS Organization, covering the full set of RCP-supported services:

*STS (`GRSTSDPB`)*::
Prevents external principals (outside the AWS Organization) from assuming IAM roles in member accounts via `sts:AssumeRole`. This closes a critical identity perimeter gap: an overly permissive role trust policy could otherwise allow a principal outside your organization to assume a role in your accounts.

*S3 (`GRS3DPB`)*::
Prevents external principals from writing to, deleting from, or modifying S3 buckets and replication configuration.

*SQS (`GRSQSDPB`)*::
Prevents external principals from sending to, receiving from, or administering SQS queues.

*Secrets Manager (`GSMDPB`)*::
Prevents external principals from creating, modifying, or deleting secrets—protecting credentials and API keys from exfiltration.

All four statements use the same condition pattern: deny unless `aws:PrincipalOrgID` matches your organization, with an exemption for AWS service principals (`aws:PrincipalIsAWSService`). This means AWS services acting on your behalf—including IAM Identity Center, Control Tower, and other managed services—are not affected.

==== Confused Deputy Protection (`GRECDP`)

Prevents AWS services from acting on your resources without a verifiable source ARN in your organization. This addresses the link:https://docs.aws.amazon.com/IAM/latest/UserGuide/confused-deputy.html[confused deputy problem], where a service with broad permissions could be tricked into acting on behalf of an external party. Applies to S3, SQS, KMS, Secrets Manager, and STS.

==== Transport Security (`GRRST`)

Denies all non-TLS requests to S3, SQS, KMS, Secrets Manager, and STS, ensuring data in transit is always encrypted.

==== Additional Protections

*KMS Key Protection (`GRKMS`)*::
Prevents external principals from modifying or deleting LZA-managed KMS keys, protecting the encryption foundation of the landing zone.

*Log Archive Protection (`GRS3OBJ`)*::
Restricts access to the Control Tower log archive bucket to CloudTrail and Config only, preserving the integrity of the audit trail.

[IMPORTANT]
====
The STS identity perimeter statement (`GRSTSDPB`) blocks cross-account role assumption from any principal outside your organization. Before deploying broadly, use link:https://docs.aws.amazon.com/IAM/latest/UserGuide/what-is-access-analyzer.html[IAM Access Analyzer] to review existing external access findings and confirm no legitimate cross-account patterns—such as third-party tool integrations or partner trust relationships—would be affected. Deploy to a test OU first and validate with CloudTrail before expanding to production OUs.
====

=== AWS CloudFormation Stack Policy Protection

link:https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/protect-stack-resources.html[AWS CloudFormation Stack Policies] provide an additional layer of infrastructure protection by preventing accidental replacement or deletion of critical networking resources during stack updates. While SCPs and RCPs control who can perform actions and what can be done to resources, stack policies control what CloudFormation itself is allowed to modify within a stack update operation.

==== Why Stack Policies

LZA deploys foundational networking infrastructure that workloads across the organization depend on. An unintentional `Update:Replace` or `Update:Delete` during a stack update could cause widespread connectivity loss, DNS resolution failures, or security inspection bypass. Stack policies act as a fail-safe mechanism that complements IAM and SCP controls:

* *SCPs* prevent unauthorized identities from making changes
* *RCPs* prevent unauthorized access to resources regardless of identity
* *Stack Policies* prevent CloudFormation from replacing or deleting critical networking resources even when the identity has permission to do so

This defense-in-depth approach ensures that even authorized pipeline operations cannot accidentally destroy infrastructure that would cause cascading failures.

==== Implementation

The LZA Universal Configuration enables stack policies in `global-config.yaml` with a deny-by-default approach for protected resource types. The policy:

* Explicitly denies `Update:Replace` and `Update:Delete` operations for protected resource types
* Allows `Update:Modify` operations (in-place updates that do not destroy the resource)
* Allows all operations on non-protected resource types

==== Protected Resource Types

The configuration protects 27 critical networking resource types across four categories:
...
|*VPC Infrastructure*
|VPC, Subnet, VPCCidrBlock, SecurityGroup, FlowLog, VPCGatewayAttachment, VPCEndpoint

|*Routing*
|RouteTable, Route, SubnetRouteTableAssociation, InternetGateway, NatGateway, PrefixList

|*Transit Gateway*
|TransitGateway, TransitGatewayVpcAttachment, TransitGatewayRouteTable, TransitGatewayRoute, TransitGatewayRouteTableAssociation, TransitGatewayRouteTablePropagation, TransitGatewayPeeringAttachment

|*Network Security and Shared Services*
|NetworkFirewall::Firewall, NetworkFirewall::FirewallPolicy, NetworkFirewall::LoggingConfiguration, NetworkFirewall::RuleGroup, RAM::ResourceShare, Route53Resolver::ResolverEndpoint, Route53Resolver::ResolverRule
|===

==== Operational Considerations

* Stack policies only protect resources during CloudFormation stack updates. Direct API, CLI, or Console modifications are governed by SCPs and IAM policies.
* Stack policies do not prevent deletion of an entire stack. Use `terminationProtection: true` (also enabled in this configuration) to prevent stack deletion.
* To update a protected resource, refer to link:../07-Operations/07-11-Managing-Stack-Policies.adoc[Managing Stack Policies] for the detailed procedure.
* Only LZA-managed stacks are affected. Stacks created outside of LZA are not impacted.
